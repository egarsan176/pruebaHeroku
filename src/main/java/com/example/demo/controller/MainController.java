	 ...
	 */
	 ...
	 public String mostrarPedido(Model model,
		 ...
		 if(this.sesion.getAttribute(usuarioString) != null) {
			 ...
			 if(contador>0) {
				 //creo un nuevo pedido a través del mapa que genera el servicioPedido al pasarle la lista de cantidades
				 Pedido pedido = new Pedido(this.servicioPedido.addProducto(listaCantidades));
				 
				 Usuario usu = (Usuario) this.sesion.getAttribute(usuarioString);
				 
				 pedido.setCosteTotalPedido(this.servicioPedido.calcularPrecioTotal());	//seteo el coste del pedido
